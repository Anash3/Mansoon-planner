...
async def get_travel_recommendation(req: TravelRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to analyze travel route: {str(e)}")
...
async def get_safety_rules(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate safety recommendations: {str(e)}")
